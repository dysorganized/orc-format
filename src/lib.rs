...
impl<F: Read+Seek> ORCFile<F> {
    ...
    fn read_compressed(&mut self, rng: std::ops::Range<u64>) -> OrcResult<Bytes> {
...
impl<'t> ColumnRef<'t> {
    ...
    pub fn read_from<F: Read+Seek>(&self, file: &mut ORCFile<F>) -> OrcResult<()> {
        if let ColumnSpec::Int{data} = &self.content {
            let start = self.stripe.info.offset() + self.stripe.info.index_length() + data.1.start;
            ...
            match prim_stream {
                PrimitiveStream::IntRLE2(idec) => println!("Got ints {:?}", idec.take(self.stripe.number_of_rows() as usize).collect::<OrcResult<Vec<i128>>>()?),
...
mod tests {
    ...
    fn read_column() {
        ...
        for cref in column_refs {
            cref.read_from(&mut orc_toc).unwrap_or_else(|e| println!("Sorry. {}", e));
        }
    }
}
