...
fn go_read_test1() {
    ...
    assert_eq!(
        frame.columns["boolean1"].valid().to_booleans(),
        Some(vec![false, true])
    );
    assert_eq!(
        frame.columns["byte1"].valid().to_numbers::<u8>().unwrap(),
        vec![1u8, 100]
    );
    assert_eq!(
        frame.columns["short1"].valid().to_numbers::<i16>().unwrap(),
        vec![1024i16, 2048]
    );
    assert_eq!(
        frame.columns["int1"].valid().to_numbers::<i32>().unwrap(),
        vec![65536i32, 65536]
    );
    assert_eq!(
        frame.columns["long1"].valid().to_numbers::<i64>().unwrap(),
        vec![9223372036854775807i64, 9223372036854775807]
    );
    assert_eq!(
        frame.columns["float1"].valid().to_numbers::<f32>().unwrap(),
        vec![1f32, 2.]
    );
    assert_eq!(
        frame.columns["double1"].valid().to_numbers::<f64>().unwrap(),
        vec![-15f64, -5.]
    );
    assert_eq!(
        frame.columns["bytes1"].valid().as_slices().unwrap(),
        vec![&[0, 1, 2, 3, 4][..], &[]]
    );
    assert_eq!(
        frame.columns["bytes1"].valid().to_vecs().unwrap(),
        vec![vec![0, 1, 2, 3, 4], vec![]]
    );
    assert_eq!(
        frame.columns["string1"].valid().as_strs().unwrap(),
        vec!["hi", "bye"]
    );
    assert_eq!(
        frame.columns["string1"].valid().to_strings().unwrap(),
